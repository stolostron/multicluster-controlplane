...
func createAggregatorServer(aggregatorConfig *aggregatorapiserver.Config, delegateAPIServer genericapiserver.DelegationTarget, apiExtensionInformers apiextensionsinformers.SharedInformerFactory, clientCert, clientKey string) (*aggregatorapiserver.APIAggregator, error) {
	...
	klog.Info("enable ocm addon management")
	...
	if err := aggregatorServer.GenericAPIServer.AddPostStartHook("multicluster-controlplane-cluster-management-addons", func(context genericapiserver.PostStartHookContext) error {
		go func() {
			ctx := goContext(context)
			crds.WaitForOcmAddonCrdsReady(ctx, dynamicClient)
			if err := controller.InstallClusterManagmentAddons(
				ctx,
				controllerConfig,
				kubeClient,
				dynamicClient,
	...
	if err := aggregatorServer.GenericAPIServer.AddPostStartHook("multicluster-controlplane-managed-cluster-addons", func(context genericapiserver.PostStartHookContext) error {
		go func() {
			ctx := goContext(context)
			crds.WaitForOcmAddonCrdsReady(ctx, dynamicClient)
			if err := controller.InstallManagedClusterAddons(
				ctx,
				controllerConfig,
				kubeClient,
			); err != nil {
