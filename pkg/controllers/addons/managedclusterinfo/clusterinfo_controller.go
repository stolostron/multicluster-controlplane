...
func SetupWithManager(mgr manager.Manager, logCertSecret string) error {
	...
	if len(logCertSecret) != 0 {
		...
		if logCertSecretNamespace == "" {
			logCertSecretNamespace, err = helpers.GetComponentNamespace()
			if err != nil {
				return err
			}
		}
...
func (r *ClusterInfoReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !cluster.GetDeletionTimestamp().IsZero() {
		...
		if helpers.ContainsString(cluster.GetFinalizers(), clusterFinalizerName) {
			...
			cluster.Finalizers = helpers.RemoveString(cluster.ObjectMeta.Finalizers, clusterFinalizerName)
	...
	if helpers.ClusterIsOffLine(clusterConditions) {
