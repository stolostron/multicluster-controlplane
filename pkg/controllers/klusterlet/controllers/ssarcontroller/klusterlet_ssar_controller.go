...
func checkBootstrapSecret(ctx context.Context, kubeClient kubernetes.Interface, agent klusterletAgent) metav1.Condition {
	bootstrapHubKubeConfigSecret := helpers.BootstrapHubKubeConfigSecret(agent.klusterlet)
	// Check if bootstrap secret exists
	bootstrapSecret, err := kubeClient.CoreV1().Secrets(agent.namespace).Get(ctx, bootstrapHubKubeConfigSecret, metav1.GetOptions{})
	if err != nil {
		return metav1.Condition{
			Status:  metav1.ConditionTrue,
			Reason:  "BootstrapSecretMissing",
			Message: fmt.Sprintf("Failed to get bootstrap secret %s/%s: %v", agent.namespace, bootstrapHubKubeConfigSecret, err),
		}
	}
	...
	if err != nil {
		return metav1.Condition{
			...
			Message: fmt.Sprintf("Failed to build bootstrap kube client with bootstrap secret %s/%s to apiserver %s: %v",
				agent.namespace, bootstrapHubKubeConfigSecret, host, err),
		}
	}
	...
	if err != nil {
		return metav1.Condition{
			Status: metav1.ConditionTrue,
			Reason: "BootstrapSecretError",
			Message: fmt.Sprintf("Failed to create %+v with bootstrap secret %s/%s: %v",
				failedReview, agent.namespace, bootstrapHubKubeConfigSecret, err),
		}
	}
	if !allowed {
		return metav1.Condition{
			...
			Message: fmt.Sprintf("Operation for resource %+v is not allowed with bootstrap secret %s/%s to apiserver %s",
				failedReview.Spec.ResourceAttributes, agent.namespace, bootstrapHubKubeConfigSecret, host),
		}
	}
	...
	return metav1.Condition{
		...
		Message: fmt.Sprintf("Bootstrap secret %s/%s to apiserver %s is configured correctly",
			agent.namespace, bootstrapHubKubeConfigSecret, host),
	}
}
...
func checkHubConfigSecret(ctx context.Context, kubeClient kubernetes.Interface, agent klusterletAgent) metav1.Condition {
	hubkubeConfigSecret := helpers.HubKubeConfigSecret(agent.klusterlet)
	hubConfigSecret, err := kubeClient.CoreV1().Secrets(agent.namespace).Get(ctx, hubkubeConfigSecret, metav1.GetOptions{})
	if err != nil {
		return metav1.Condition{
			Status:  metav1.ConditionTrue,
			Reason:  "HubKubeConfigSecretMissing",
			Message: fmt.Sprintf("Failed to get hub kubeconfig secret %s/%s: %v", agent.namespace, hubkubeConfigSecret, err),
		}
	}
	...
	if hubConfigSecret.Data["kubeconfig"] == nil {
		return metav1.Condition{
			...
			Message: fmt.Sprintf("Failed to get kubeconfig from `kubectl get secret -n %s/%s -ojsonpath='{.data.kubeconfig}'`. "+
	...
	if err != nil {
		return metav1.Condition{
			...
			Message: fmt.Sprintf("Failed to build hub kube client with hub config secret %s/%s: %v",
				hubConfigSecret.Namespace, hubConfigSecret.Name, err),
		}
	}

	clusterName := agent.klusterlet.Spec.ClusterName
	...
	if clusterName == "" {
		if hubConfigSecret.Data["cluster-name"] == nil {
			return metav1.Condition{
				...
				Message: fmt.Sprintf(
					"Failed to get cluster name from `kubectl get secret -n %s/%s -ojsonpath='{.data.cluster-name}`."+
	...
	if err != nil {
		return metav1.Condition{
			Status: metav1.ConditionTrue,
			Reason: "HubKubeConfigError",
			Message: fmt.Sprintf("Failed to create %+v with hub config secret %s/%s to apiserver %s: %v",
	...
	if !allowed {
		return metav1.Condition{
			...
			Message: fmt.Sprintf("Operation for resource %+v is not allowed with hub config secret %s/%s to apiserver %s",
	...
	return metav1.Condition{
		...
		Message: fmt.Sprintf("Hub kubeconfig secret %s/%s to apiserver %s is working",
			agent.namespace, hubkubeConfigSecret, host),
	}
}
