...
func (k *bootstrapController) sync(ctx context.Context, controllerContext factory.SyncContext) error {
	...
	bootstrapSecret := helpers.BootstrapHubKubeConfigSecret(klusterlet)
	bootstrapHubKubeconfigSecret, err := k.secretLister.Secrets(agentNamespace).Get(bootstrapSecret)
	switch {
	...
	if err != nil {
		...
		controllerContext.Recorder().Warningf("BadBootstrapSecret",
			fmt.Sprintf("unable to load hub kubeconfig from secret %s/%s: %v", agentNamespace, bootstrapSecret, err))
		return nil
	}

	hubKubeConfig := helpers.HubKubeConfigSecret(klusterlet)
	hubKubeconfigSecret, err := k.secretLister.Secrets(agentNamespace).Get(hubKubeConfig)
	switch {
	case errors.IsNotFound(err):
	...
	if err != nil {
		...
		controllerContext.Recorder().Warningf("BadHubKubeConfigSecret",
			fmt.Sprintf("unable to load hub kubeconfig from secret %s/%s: %v", agentNamespace, hubKubeConfig, err))
		return nil
	}
	...
	if bootstrapKubeconfig.Server != hubKubeconfig.Server ||
		...
		reloadReason := fmt.Sprintf("the bootstrap secret %s/%s is changed", agentNamespace, bootstrapSecret)
		return k.reloadAgents(ctx, controllerContext, agentNamespace, hubKubeConfig, klusterletName, reloadReason)
	}
	...
	if err != nil {
		// the hub kubeconfig secret has errors, do nothing
		controllerContext.Recorder().Warningf("BadHubKubeConfigSecret",
			fmt.Sprintf("the hub kubeconfig secret %s/%s is invalid: %v", agentNamespace, hubKubeConfig, err))
		return nil
	}
	...
	reloadReason := fmt.Sprintf("the hub kubeconfig secret %s/%s is expired", agentNamespace, hubKubeConfig)
	return k.reloadAgents(ctx, controllerContext, agentNamespace, hubKubeConfig, klusterletName, reloadReason)
}
...
func (k *bootstrapController) reloadAgents(ctx context.Context, ctrlContext factory.SyncContext, namespace, hubKubeConfig, klusterletName, reason string) error {
	...
	ctrlContext.Recorder().Eventf("HubKubeconfigSecretDeleted", fmt.Sprintf("the hub kubeconfig secret %s/%s is deleted due to %s",
		namespace, hubKubeConfig, reason))

	agentName := fmt.Sprintf("%s-multicluster-controlplane-agent", klusterletName)
	if err := k.kubeClient.AppsV1().Deployments(namespace).Delete(ctx, agentName, metav1.DeleteOptions{}); err != nil {
		return err
	}
	ctrlContext.Recorder().Eventf("KlusterletAgentDeploymentDeleted", fmt.Sprintf("the deployment %s/%s is deleted due to %s",
		namespace, agentName, reason))
...
func bootstrapSecretQueueKeyFunc(klusterletLister operatorlister.KlusterletLister) factory.ObjectQueueKeyFunc {
	return func(obj runtime.Object) string {
		...
		if name != helpers.BootstrapHubKubeConfig && name != helpers.ControlplaneBootstrapHubKubeConfig {
			return ""
		}
