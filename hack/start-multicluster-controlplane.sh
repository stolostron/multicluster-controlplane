...
FEATURE_GATES=${FEATURE_GATES:-"DefaultClusterSet=true,OpenAPIV3=false,AddonManagement=true"}

# enable self managed
ENABLE_SELF_MANAGED=${ENABLE_SELF_MANAGED:-"false"}

# apiserver port
APISERVER_PORT=${APISERVER_PORT:-"9443"}
...
function test_apiserver_off {
    ...
    if ! curl --silent -k -g "${EXTERNAL_HOSTNAME}:${APISERVER_PORT}" ; then
        echo "API SERVER secure port is free, proceeding..."
    else
        echo "ERROR starting API SERVER, exiting. Some process on ${EXTERNAL_HOSTNAME} is serving already on ${apiserver_port}"
        exit 1
    fi
}
...
if [ ! -f "${config_dir}/ocmconfig.yaml" ] ; then
    if [[ "$(uname)" == "Darwin" ]]; then
        EXTERNAL_HOSTNAME=$(ifconfig en0 | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\.){3}[0-9]*')
    else
        EXTERNAL_HOSTNAME=$(ifconfig eth0 | grep -Eo 'inet (addr:)?([0-9]*\.){3}[0-9]*' | grep -Eo '([0-9]*\.){3}[0-9]*')
    fi
