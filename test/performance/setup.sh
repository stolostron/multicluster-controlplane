...
for j in $(seq 1 "${HOSTED_CLUSTER_NUMBER}"); do
  kind delete cluster --name controlplane-hosted-mc$j
done

rm -rf $REPO_DIR/_output
mkdir -p $REPO_DIR/_output
...
external_host_port="30081"

helm --kubeconfig $kubeconfig upgrade --install \
...
for j in $(seq 1 "${HOSTED_CLUSTER_NUMBER}"); do
  hosted_cluster_name="controlplane-hosted-mc$j"
...
#kubectl --kubeconfig ${hubkubeconfig} apply -f ${REPO_DIR}/test/performance/compliant-policies
